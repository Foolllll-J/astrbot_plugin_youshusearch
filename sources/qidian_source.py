...
class QidianSource:
    def __init__(
        self, session_factory: Callable[..., aiohttp.ClientSession] | None = None
    ):
        self.headers = {
    ...
    async def search_book(self, keyword, page=1, return_metadata=False):
        ...
        async with self.session_factory(headers=self.headers) as session:
            try:
                ...
                logger.info(
                    f"🔍 [优书搜索] 正在获取起点正版数据 (仅第1页): {search_url}"
                )

                async with session.get(search_url, timeout=10) as resp:
                    content = await resp.text()
                    tree = html.fromstring(content)
                    script_node = tree.xpath(
                        "//script[@id='vite-plugin-ssr_pageContext']/text()"
                    )
                ...
                return (
                    {"books": [], "total": 0, "current_page": page, "is_last": True}
                    if return_metadata
                    else []
                )
                ...
                async with session.get(book_url, timeout=10) as resp:
                    ...
                    if script_node:
                        try:
                            ...
                            tags = [
                                t.get("TagName")
                                for t in book_extra.get("ugcTagInfos", [])
                                if t.get("TagName")
                            ]
                            ...
                            raw_chapter_content = chapter_data.get(
                                "firstChapterC", ""
                            ).strip()
                            formatted_content = (
                                "　　" + raw_chapter_content
                                if raw_chapter_content
                                else ""
                            )
                            ...
                            return {
                                ...
                                "first_chapter_title": chapter_data.get(
                                    "firstChapterT"
                                ),
