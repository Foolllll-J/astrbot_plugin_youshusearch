...
class SearchStateManager:
    ...
    def get_item_by_number(
        self, user_id: str, number: int, search_type: str
    ) -> Optional[Dict]:
        """根据序号和搜索类型获取书籍信息"""
        state = self.get_state(user_id)
...
from .sources.youshu_source import YoushuSource, YS_API1_HEADERS, YS_API2_HEADERS  # noqa: E402
from .sources.uaa_source import UaaSource  # noqa: E402
from .sources.qidian_source import QidianSource  # noqa: E402
from .core.http_session import build_session_factory  # noqa: E402
...
class YoushuSearchPlugin(Star):
    def __init__(self, context: Context, config=None):
        ...
        self.config = config
        self.session_factory: Callable[..., aiohttp.ClientSession] = (
    ...
    def _render_search_results(
        ...
        for i, book in enumerate(results.books):
            ...
            scorer_info = (
                f" ({book.scorer}人)" if book.scorer and book.scorer != "0" else ""
            )
    ...
    async def _get_enriched_book_details(
        self, source, session, novel_id: str, title: Optional[str] = None
    ) -> Optional[Book]:
        ...
        if self.enable_official_metadata and source == self.youshu_source:
            try:
                ...
                if match_book:
                    # 3. 获取起点详情
                    q_details = await self.qidian_source.get_book_details(
                        match_book["url"]
                    )
                    if q_details:
                        # 4. 覆盖元数据（排除评分和评分人数）
                        if q_details.get("author"):
                            book.author = q_details["author"]
                        if q_details.get("status"):
                            book.status = q_details["status"]
                        if q_details.get("category"):
                            book.category = q_details["category"]
                        if q_details.get("tags"):
                            book.tags = q_details["tags"]
                        if q_details.get("word_count"):
                            book.word_count = q_details["word_count"]
                        if q_details.get("last_update"):
                            book.update_time = q_details["last_update"]
                        if q_details.get("last_chapter"):
                            book.last_chapter = q_details["last_chapter"]
                        if q_details.get("intro"):
                            book.synopsis = q_details["intro"]
                        if q_details.get("cover"):
                            book.image_url = q_details["cover"]
    ...
    async def _render_book_details(self, event: AstrMessageEvent, book: Book):
        ...
        if book.word_count is not None:
            if isinstance(book.word_count, str) and (
                "K" in book.word_count or "M" in book.word_count
            ):
                message_text += f"📏 字数: {book.word_count}\n"
            else:
                try:
                    message_text += (
                        f"📏 字数: {float(book.word_count) / 10000:.2f}万字\n"
                    )
                except Exception:
                    message_text += f"📏 字数: {book.word_count}\n"
        ...
        if book.reviews:
            ...
            for review in book.reviews[:5]:  # 最多显示5条
                ...
                try:
                    rating_str = (
                        f"{float(rating):.2f}" if rating and rating != "无" else "无"
                    )
                except (ValueError, TypeError):
        ...
        if book.image_url:
            try:
                ...
                async with self.session.get(
                    book.image_url, timeout=timeout
                ) as img_response:
    ...
    async def _handle_next_page(self, event: AstrMessageEvent, search_type: str):
        ...
        if not state.get("keyword") or state.get("search_type") != search_type:
            yield event.plain_result(
                f"🤔 没有可供翻页的搜索结果，请先使用 /{search_type} 书名 进行搜索。"
            )
            return
        ...
        try:
            ...
            message_text = self._render_search_results(
                keyword, search_result, next_page, search_type
            )
    ...
    async def _handle_prev_page(self, event: AstrMessageEvent, search_type: str):
        ...
        try:
            ...
            message_text = self._render_search_results(
                keyword, search_result, prev_page, search_type
            )
            yield event.plain_result(message_text)
        except Exception as e:
            logger.error(f"翻页失败: {e}", exc_info=True)
    ...
    async def youshu_search_command(self, event: AstrMessageEvent):
        ...
        if sub_cmd in ("next", "n", "下一页"):
            async for res in self._handle_next_page(event, "ys"):
                yield res
            return
        elif sub_cmd in ("prev", "p", "上一页"):
            async for res in self._handle_prev_page(event, "ys"):
                yield res
            return
        ...
        logger.info(
            f"用户 {user_id} 触发 /ys, 搜索:'{book_name}', 序号:{item_index}, 列表页:{page_to_list}"
        )

        try:
            # 搜索书籍
            search_result = await self.youshu_source.search(
                self.session, book_name, page_to_list
            )
            ...
            if page_to_list > search_result.total_pages > 0:
                yield event.plain_result(
                    f"❌ 您请求的第 {page_to_list} 页不存在，【{book_name}】的搜索结果共 {search_result.total_pages} 页。"
                )
                return
            ...
            if (
                item_index is None
                and len(search_result.books) == 1
                and search_result.total_pages == 1
            ):
                # 如果只有一页且只有一个结果，直接显示详情
                selected_book = search_result.books[0]
                book_details = await self._get_enriched_book_details(
                    self.youshu_source,
                    self.session,
                    selected_book.id,
                    selected_book.title,
            ...
            if item_index is None:
                # 显示搜索结果列表
                message_text = self._render_search_results(
                    book_name, search_result, page_to_list, "ys"
                )
                yield event.plain_result(message_text)
            else:
                # 显示特定书籍的详情
                ...
                if correct_page != page_to_list:
                    yield event.plain_result(
                        f"⏳ 序号【{item_index}】位于第 {correct_page} 页，正在为您跳转..."
                    )
                    search_result = await self.youshu_source.search(
                        self.session, book_name, correct_page
                    )
                    if search_result is None or not search_result.books:
                        yield event.plain_result(
                            f"😢 未在第 {correct_page} 页找到关于【{book_name}】的信息。"
                        )
                        return
                    # 更新状态至正确页面
                    self.state_mgr.update_state(
                ...
                if not (0 <= index_on_page < len(search_result.books)):
                    yield event.plain_result(
                        f"❌ 序号【{item_index}】在第 {correct_page} 页上不存在。"
                    )
                    return
    ...
    async def hs_search_command(self, event: AstrMessageEvent):
        ...
        sub_cmd = args[0].lower()
        if sub_cmd in ("next", "n", "下一页"):
            async for res in self._handle_next_page(event, "hs"):
                yield res
            return
        elif sub_cmd in ("prev", "p", "上一页"):
            async for res in self._handle_prev_page(event, "hs"):
                yield res
            return
        ...
        logger.info(
            f"用户 {user_id} 触发 /hs, 搜索:'{book_name}', 序号:{item_index}, 列表页:{page_to_list}"
        )

        try:
            # 搜索书籍
            search_result = await self.uaa_source.search(
                self.session, book_name, page_to_list
            )
            if search_result is None:
                yield event.plain_result(
            ...
            if (
                ...
                selected_book = search_result.books[0]
                book_details = await self._get_enriched_book_details(
                    self.uaa_source, self.session, selected_book.id, selected_book.title
                )
                if book_details:
                    async for result in self._render_book_details(event, book_details):
                        yield result
            ...
            if item_index is None:  # 显示列表
                message_text = self._render_search_results(
                    book_name, search_result, page_to_list, "hs"
                )
                yield event.plain_result(message_text)
            else:  # 显示详情
                results_per_page = 20
    ...
    async def _get_latest_novel_id(self) -> Optional[int]:
        ...
        try:
            ...
            async with self.session.get(url, headers=headers, timeout=10) as response:
                ...
                # 在 HTML 中查找小说 ID
                matches = re.findall(
                    r'href="/novel/(\d+)\.html"|href="/book/(\d+)"', html_content
                )
                if matches:
    ...
    async def youshu_random_command(self, event: AstrMessageEvent):
        ...
        for attempt in range(max_retries):
            ...
            try:
                book_details = await self._get_enriched_book_details(
                    self.youshu_source, self.session, str(random_id)
                )
                if book_details:
                    async for result in self._render_book_details(event, book_details):
                        yield result
